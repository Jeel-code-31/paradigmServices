  ...
  return (
    <main className="min-h-screen bg-paradigm-bg">
      ...
      <section className="relative overflow-hidden pt-20 pb-12">
        <div className="absolute inset-x-1/2 -top-10 h-36 w-36 -translate-x-1/2 rounded-full bg-[#BAC291]/30 blur-3xl" />
        <div className="relative max-w-4xl mx-auto px-6 text-center">
          <div className="inline-flex items-center justify-center gap-3 mb-6">
            <span className="block h-[2px] w-12 rounded-full bg-[#BAC291]" />
            <span style={{ color: COLORS.primary }} className="text-sm uppercase tracking-[0.2em] font-bold">
              Our Work
            </span>
            <span className="block h-[2px] w-12 rounded-full bg-[#BAC291]" />
          </div>
          <h2 style={{ color: COLORS.primary }} className="text-4xl md:text-5xl font-black mb-6 leading-tight">
            Our Clients & Impact
          </h2>
          <p className="max-w-3xl font-afaca mx-auto text-lg md:text-xl">
        ...
        {clientProjects.map((project, index) => {
          ...
          return (
            <motion.div 
              ...
              className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center rounded-3xl bg-[#1A3013] shadow-[0_20px_50px_-20px_rgba(26,48,19,0.3)] p-6 lg:p-10 relative overflow-hidden"
            >
              ...
              {/* TEXT CONTENT */}
              <div className={`order-2 z-10 ${isImageLeft ? 'lg:order-2' : 'lg:order-1'}`}>
                <div className="flex items-center gap-3 mb-4">
                  <div style={{ backgroundColor: COLORS.accent }} className="h-[2px] w-8" />
                  <span style={{ color: COLORS.accent }} className="font-bold uppercase tracking-widest text-sm">
                    {project.category}
                  </span>
                </div>
                
                <h3 className="text-white text-3xl md:text-4xl font-black mb-6 leading-tight">
                  {project.title}
                </h3>
                ...
                <button 
                  ...
                  className="group inline-flex items-center gap-3 rounded-full px-8 py-4 font-afaca shadow-lg transition-all duration-300 hover:bg-white hover:scale-[1.02]"
                >
                  See More Details
              ...
              <div className={`order-1 relative h-64 sm:h-80 lg:h-[400px] w-full z-10 ${isImageLeft ? 'lg:order-1' : 'lg:order-2'}`}>
                <div className="relative w-full h-full overflow-hidden rounded-2xl cursor-pointer shadow-2xl" onClick={() => setSelectedProject(project)}>
        ...
        {selectedProject && (
          ...
          >
            ...
            <div className="sticky top-0 bg-white/90 backdrop-blur-md z-20 border-b border-gray-100 px-6 py-4 flex justify-between items-center">
              <span style={{ color: COLORS.primary }} className="font-bold tracking-widest uppercase text-sm flex items-center gap-2">
              ...
              >
                <X size={24} style={{ color: COLORS.primary }} className="hover:text-current" />
              </button>
            </div>
            ...
            <div className="max-w-[1400px] mx-auto px-6 py-12 lg:py-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-16 lg:mb-24">
                {selectedProject.gallery.map((img, idx) => (
                  <div 
                    key={img.id} 
                    className={`${idx === 0 ? 'md:col-span-2 aspect-[21/9]' : 'aspect-[4/3]'} w-full overflow-hidden rounded-2xl bg-[#F7F9EF] shadow-md`}
                  >
                    <img 
                      src={img.src} 
                      alt={img.alt} 
              ...
              <div className="max-w-4xl mx-auto text-center">
                <h3 style={{ color: COLORS.primary }} className="text-3xl md:text-5xl font-black mb-6">
                  {selectedProject.title}
                </h3>
                
                <div className="font-afaca text-lg md:text-xl text-gray-700 mb-6">
                  {selectedProject.description}
                </div>

                {/* Conditional Rendering for Highlights */}
                {selectedProject.Highlight && (
                  <div className="font-afaca text-lg md:text-xl text-center text-gray-700 p-6 rounded-xl inline-block mt-4 text-left">
                    <span className="font-bold block mb-2">Project Highlights:</span>
                    {selectedProject.Highlight}
                  </div>
                )}
              </div>
